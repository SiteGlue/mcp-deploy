...
// Vapi-compatible endpoints that return data directly
app.post('/vapi/tools/call', async (req, res) => {
  try {
    ...
    if (method === 'tools/call') {
      ...
      if (name === 'findMedRehabLocation') {
        ...
        switch (tool_name) {
          case 'findLocationByPostalCode':
            ...
            const locationResult = await juvonnoServer.findLocationByPostalCode(postal_code);
            console.log('Location result:', JSON.stringify(locationResult, null, 2));
            return res.json(locationResult);
            
          case 'getAllLocations':
            const allLocationsResult = await juvonnoServer.getAllLocations();
            console.log('All locations result:', JSON.stringify(allLocationsResult, null, 2));
            return res.json(allLocationsResult);
      ...
      switch (name) {
        case 'findLocationByPostalCode':
          const directLocationResult = await juvonnoServer.findLocationByPostalCode(args.postal_code);
          return res.json(directLocationResult);
          
        case 'getAllLocations':
          const directAllLocationsResult = await juvonnoServer.getAllLocations();
          return res.json(directAllLocationsResult);
...
app.options('/vapi/tools/call', (req, res) => {
  res.header('Access-Control-Allow-Origin', '*');
  res.header('Access-Control-Allow-Methods', 'POST, OPTIONS');
  res.header('Access-Control-Allow-Headers', 'Content-Type, Authorization');
  res.status(200).send();
